...
GICBSSearch::GICBSSearch(const MapLoader& ml, const AgentsLoader& al, double f_w, const EgraphReader& egr, constraint_strategy c, int prior_strategy): focal_w(f_w), prior_strategy(prior_strategy)
{
	cons_strategy = c;
	//focal_w = f_w;
	HL_num_expanded = 0;
	...
	vector<size_t> idx(num_of_agents);
	...
	if (prior_strategy >= 1) {
		...
		if (prior_strategy == 1) {
			sort(idx.begin(), idx.end(),
				[&](size_t i1, size_t i2) {return search_engines[i1]->my_heuristic[search_engines[i1]->start_location] > search_engines[i2]->my_heuristic[search_engines[i2]->start_location];});
		}
		else if (prior_strategy == 2) {
			sort(idx.begin(), idx.end(),
				[&](size_t i1, size_t i2) {return search_engines[i1]->my_heuristic[search_engines[i1]->start_location] < search_engines[i2]->my_heuristic[search_engines[i2]->start_location];});
		}
	}
	...
	for (int i = 0; i < num_of_agents - 1; i++) {
		dummy_start->priorities[idx[i]][idx[i + 1]] = true;
		for (int j = i; j < num_of_agents - 1; j++) {
			dummy_start->trans_priorities[idx[i]][idx[j + 1]] = true;
		}
	}
	...
	for (int i = 0; i < num_of_agents; i++) {
		...
		if (search_engines[idx[i]]->findPath(paths_found_initially[idx[i]], f_w, dummy_start->trans_priorities, paths, dummy_start->makespan + 1, 0) == false) {
			cout << "NO SOLUTION EXISTS";
			solution_cost = -2;
			break;
		}
		//dummy_start->paths[i] = search_engines[i]->getPath();
		paths[idx[i]] = &paths_found_initially[idx[i]];
		dummy_start->makespan = max(dummy_start->makespan, paths_found_initially[idx[i]].size() - 1);
		...
		LL_num_expanded += search_engines[idx[i]]->num_expanded;
		LL_num_generated += search_engines[idx[i]]->num_generated;
		//delete[] res_table;
		//    cout << endl;
	}
	...
	if (solution_cost != -2) {
		...
		dummy_start->g_val = 0;
		for (int i = 0; i < num_of_agents; i++)
			dummy_start->g_val += paths[idx[i]]->size() - 1;
		dummy_start->h_val = 0;
