	...
	if (true) {
		...
		while (!q.empty() && foundSol) {
			...
			if (foundSol) {
				...
				node->g_val = node->g_val - paths[curr_agent]->size() + newPath.second.size();
				...
				for (int i = 0; i < num_of_agents; i++) {
					if (node->priorities[curr_agent][i] && i != curr_agent) {
						q.push(i);
					}
				}
			}
...
bool GICBSSearch::runGICBSSearch() 
{
	...
	while (!open_list.empty() && !solution_found) 
	{
		...
		{  // timeout after 1 minutes
			...
			std::cout << "Depth of last node: " << open_list.top()->depth << " ; MaxDepth = " << maxDepth <<  std::endl;
...
//#endif		
		...
		if (gen_n1) {
			...
			for (int i = 0; i < num_of_agents; i++) { // transitivity
				if (n1->trans_priorities[n1->agent_id][i]) { // a2->a1 and a1->i => a2->i
					n1->trans_priorities[n2->agent_id][i] = true;
				}
				if (n1->trans_priorities[i][n2->agent_id]) { // a2->a1 and i->a2 =>i->a1
					n1->trans_priorities[i][n1->agent_id] = true;
				}
			}
		...
		if (gen_n2) {
			...
			for (int i = 0; i < num_of_agents; i++) { // transitivity
				if (n2->trans_priorities[n2->agent_id][i]) { // a1->a2 and a2->i => a1->i
					n2->trans_priorities[n1->agent_id][i] = true;
				}
				if (n2->trans_priorities[i][n1->agent_id]) { // a1->a2 and i->a1 =>i->a2
					n2->trans_priorities[i][n2->agent_id] = true;
				}
			}
	...
	//    printPaths();	
	if (open_list.empty() && solution_cost < 0)
	{
		solution_cost = -2;
