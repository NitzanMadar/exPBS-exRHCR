...
def assign_new_missions(agent_fname_, mew_missions):
    ...
    agents_to_update_g = []
    for i in range(k):
        if agent_start_dict[i] == agent_goal_dict[i] : #euclidean_distance(agent_start_dict[i], agent_goal_dict[i])<= 3:#agent_start_dict[i] == agent_goal_dict[i] :
...
create_top_sort_priorities = True

window_size = 10
# h = 5

# number_of_agents = 400
output_fname = folder_name + str(number_of_agents) + "agents/lifelong/Output_lifelong_sorting_"+str(number_of_agents)+"delta-"+str(J)+"+WL-DFS_w="+str(window_size)+"-h="+str(h)+"_ell="+l+"_p="+str(p)+".csv"
...
if J == 0:
    ...
    with open(output_fname, 'a') as f:
        ...
        f.write(f'test_num,{test_num}\nPBS, {number_of_agents} agents, w=h={window_size}, total steps = {total_step_limit}\n')
    f.close()
