...
window_size = 20    # RHCR, PBS only
h=5                 # RHCR, PBS only
# J = 2  # PBS --> J*exPBS  (delta)
# number_of_agents = 150
output_fname = folder_name + str(number_of_agents) + "agents/lifelong/Output_lifelong_kiva_"+str(number_of_agents)+"_PBS-"+str(J)+"exPBS+BLDFS_w="+str(window_size)+"-h="+str(h)+"_ell_effect="+l+".csv"
...
if J == 0:
    ...
    with open(output_fname, 'a') as f:
        ...
        f.write(f'PBS, {number_of_agents} agents, w={window_size}, h={h}, total steps = {total_step_limit}\n')
    f.close()
    ...
    while steps_done < total_step_limit:
        ...
        # original_pbs = './driver --map '+map_fname+' --agents ' + agent_fname_expbs + \
        #                ' --agentNum ' + str(number_of_agents) + ' --width_limit_hl 50 --fallback 1' + \
        ...
        for _ in range(J):  # run J exPBS after 1 PBS
            ...
            pbs_with_experience = prefix_dir+"driver --map "+map_fname+ \
                              ...
                              " --experience 1  --cleaning_threshold -1 --to_save_P_matrix 0 --windowed_mapf " + str(w_exPBS)+ ' --frequency_mapd ' + str(h)
