...
output_fname = 'Output_'+map_fname+'_'+number_of_agents+'agents_random_exp_choose.csv'
map_fname = map_fname + '.map'

random_list= [ random.randint(0,5000) for _ in range(number_of_queries)]
print(random_list)
            ...
            for a in range(0, number_of_queries, 1):  # number of numbered agents file to solve
                ...
                if solver == "PBS+Experience":
                    pbs_with_experience = "./driver --map " + folder_name+map_fname + " --agents " + folder_name+agents_fname + \
                                          ...
                                          + " --priorityMatrix "+str(random_list[a])
                ...
                if solver == "PBS+Experience+Clean":
                    pbs_with_experience_with_clean = "./driver --map " + folder_name+map_fname + " --agents " + \
                                                     ...
                                                     " --width_limit_hl " + str(width) + " --cleaning_threshold -2" + \
                                                     " --priorityMatrix "+str(random_list[a]) # --cleaning_threshold = -2 means clean by 1*average distance, cleaning_threshold = -1 means do not clean
