...
int main(int argc, char** argv) {
	...
	desc.add_options()
		...
		("priority,p", po::value<int>()->default_value(1), "priority branching (0 = lazy generating, 1 = aggressive generating, 2 = aggressive greedy, 3 = fixed priority)")
	;
	...
	{ 
		...
		if (vm["priority"].as<int>() >= 1) {
			GICBSSearch icbs(ml, al, 1.0, egr, s, vm["priority"].as<int>());
			bool res;
			...
			stats << icbs.runtime << "," <<
				...
				"FIX" << vm["priority"].as<int>() << endl;
			stats.close();
		}
		else {
			for (int i = 0; i < 9; i++) {
				vector<int> indices(al.num_of_agents,0);
				vector<pair<int,int>> start_locs(al.initial_locations);
				vector<pair<int, int>> goal_locs(al.goal_locations);
				std::iota(indices.begin(), indices.end(), 0);
				random_shuffle(indices.begin(), indices.end());
				for (int ag = 0; ag < al.num_of_agents; ag++) {
					al.initial_locations[ag] = start_locs[indices[ag]];
					al.goal_locations[ag] = goal_locs[indices[ag]];
				}
