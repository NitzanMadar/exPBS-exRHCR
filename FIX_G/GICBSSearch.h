...
public:
	...
	//bool fixed_prior;
	int prior_strategy; // 0 = random (do nothing), 1 = longest highest, 2 = shortest highest
	double runtime = 0;
	...
	GICBSSearch(const MapLoader& ml, const AgentsLoader& al, double f_w, const EgraphReader& egr, constraint_strategy c, int prior_strategy = 0);
	~GICBSSearch();
};
